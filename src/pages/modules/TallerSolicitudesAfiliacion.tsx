...
export default function TallerSolicitudesAfiliacion() {
  ...
  const setupRealtimeSubscription = () => {
    ...
    const channel = supabase
      .channel("solicitudes_taller_changes")
      .on(
        "postgres_changes",
        {
          event: "*",
  ...
  const handleAction = async () => {
    if (!selectedSolicitud || !tallerId) return;
    ...
    try {
      ...
      if (actionType === "aprobar") {
        const { error: relationError } = await supabase
          ...
          .insert({
            taller_id: tallerId,
            aseguradora_id: selectedSolicitud.aseguradora_id,
          });
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <CardTitle className="text-xl">
                        {solicitud.aseguradoras?.nombre_aseguradora || "Aseguradora no disponible"}
                      </CardTitle>
                    </div>
                    <CardDescription>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      📍 {solicitud.aseguradoras?.ciudad || "N/A"}, {solicitud.aseguradoras?.estado || "N/A"}
                    </p>
                    <p className="text-muted-foreground">
                      {solicitud.aseguradoras?.direccion || "N/A"}
                    </p>
                  </div>
                  <div>
                    <p className="font-medium mb-1">Contacto</p>
                    <p className="text-muted-foreground">
                      📧 {solicitud.aseguradoras?.email || "N/A"}
                    </p>
                    <p className="text-muted-foreground">
                      📞 {solicitud.aseguradoras?.telefono || "N/A"}
                    </p>
                  </div>
                </div>

                <div>
                  <p className="font-medium mb-1">RFC</p>
                  <p className="text-sm text-muted-foreground">
                    {solicitud.aseguradoras?.rfc || "N/A"}
                  </p>
                </div>

                {solicitud.aseguradoras?.descripcion && (
                  <div>
                    <p className="font-medium mb-1">Descripción de la aseguradora</p>
                ...
                {solicitud.mensaje && (
                  <div>
                    <p className="font-medium mb-1">Mensaje de la aseguradora</p>
            ...
            <Card key={solicitud.id} className="opacity-90">
              ...
              <CardContent className="space-y-4">
                <div className="text-sm">
                  <p className="text-muted-foreground">📍 {solicitud.aseguradoras?.ciudad || "N/A"}, {solicitud.aseguradoras?.estado || "N/A"}</p>
                  <p className="text-muted-foreground">📧 {solicitud.aseguradoras?.email || "N/A"}</p>
                </div>
                {solicitud.mensaje && (
                  <div>
                    <p className="text-sm font-medium mb-1">Mensaje de la aseguradora:</p>
                    <p className="text-sm text-muted-foreground bg-muted p-3 rounded-md">
                      {solicitud.mensaje}
                    </p>
                  </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {actionType === "aprobar" 
                ? `¿Estás seguro de aceptar la solicitud de ${selectedSolicitud?.aseguradoras?.nombre_aseguradora}?`
                : `¿Estás seguro de rechazar la solicitud de ${selectedSolicitud?.aseguradoras?.nombre_aseguradora}?`
              }
            </DialogDescription>
          </DialogHeader>
